...
std::enable_if_t<!ivanp::is_std_vector<std::decay_t<T>>::value,vector<V>>
operator+ (vector<V> v, T&& x) {
  v.emplace_back(std::forward<T>(x));
  return v;
}
template <typename V, typename T>
vector<V> operator+ (vector<V> v, const vector<T>& xs) {
  for (const auto& x : xs) v.emplace_back(x);
  return v;
}
